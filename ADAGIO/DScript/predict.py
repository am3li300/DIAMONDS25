...
def dscript_predict(pairs_df, modelPath, outPath, seqDict, threshold, store_cmaps=None, foldseek_fasta=None, embPath=None):
    ...
    if modelPath.endswith(".sav") or modelPath.endswith(".pt"):
        try:
            ...
            print(f"Model {modelPath} not found", file=logFile, print_also=True)
            # logFile.close()
            sys.exit(1)
    else:
        try:
            model = DSCRIPTModel.from_pretrained(
                modelPath, use_cuda=False # use_cuda
            )
            # if use_cuda:
            #     model = model.cuda()
            #     model.use_cuda = True
    ...
    if embPath is None:
        ...
        for n in tqdm(all_prots):
            embeddings[n] = lm_embed(seqDict[n], False) # use_cuda)
    else:
    ...
    if foldseek_fasta is not None:
        ...
        try:
            ...
            print(f"Foldseek Sequence File {foldseek_fasta} not found")
            # log(
                    ...
                    try:
                        if foldseek_fasta is not None:
                            try:
                                ...
                                print("Loaded model does not support foldseek. Please retrain with --allow_foldseek or download a pre-trained TT3D model.")
                                # log(e)
                                # log(
